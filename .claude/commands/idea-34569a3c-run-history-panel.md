...
- **Generated**: 11/14/2025, 9:42:53 PM

## Description
Add a new RunHistoryPanel component that queries Supabase for past runs (using createRun metadata) and displays them in a sortable table inside RealRunner. Each row shows status, duration, and a small ExecutionProgress bar. Users can click a run to re-launch or view detailed logs. Leverages existing supabase helpers and the theme system.

## Reasoning
Testers often need to review previous executions to debug failures. Providing a consolidated history inside the dashboard reduces context switching and speeds up issue triage. The feature reuses current data shapes and UI patterns, so it aligns tightly with the existing architecture.
