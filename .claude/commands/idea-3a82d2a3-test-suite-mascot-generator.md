...
## Description
Automatically generate a playful mascot or avatar that represents the test suite, using its name and key actions (e.g., a robot holding a mouse for UI tests). Display the mascot in the wizard header and in the test list, and animate it slightly on each step transition. Users can also customize the mascot’s color scheme to match their team branding.

## Reasoning
A friendly visual companion adds personality to the tool, turning routine test creation into a more engaging activity. It creates a memorable identity for each suite, helping teams feel a stronger attachment and improving recall when sharing or reviewing tests. Small delight with minimal performance impact.
